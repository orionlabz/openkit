...
fn run_agent_sync_at(project: &Path, agent: AgentName, args: AgentSyncArgs) -> Result<(), String> {
    ...
    if matches!(agent, AgentName::Antigravity) {
        if let Some(ag_cmds) = TEMPLATE_ROOT.get_dir("antigravity/commands") {
            copy_embedded_dir_with_prefix(
                ag_cmds,
                &target.join("commands"),
                args.overwrite,
                &PathBuf::from("antigravity/commands"),
            )?;
            collect_embedded_file_paths(
                ag_cmds,
                &PathBuf::from("antigravity/commands"),
...
fn run_check(args: CheckArgs) -> Result<(), String> {
    let agents = vec![
        ...
        ("Antigravity", "antigravity", vec!["--version"]),
    ];
